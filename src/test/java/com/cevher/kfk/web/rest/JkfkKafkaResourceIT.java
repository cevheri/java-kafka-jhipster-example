...
import com.cevher.kfk.config.KafkaProperties;
...
class JkfkKafkaResourceIT {
    ...
    void setup() {
        ...
        JkfkKafkaResource kafkaResource = new JkfkKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jkfk-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jkfk-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
